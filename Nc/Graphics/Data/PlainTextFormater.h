...
#ifndef NC_GRAPHICS_DATA_PLAINTEXTFORMATER_H_
#define NC_GRAPHICS_DATA_PLAINTEXTFORMATER_H_
...
//                        Justify         ///< Align the text to both left and right, adding extra space between words as necessary.
            ...
            public:
                ...
                virtual void        InitDrawables(GL::DrawableArray &drawableArray);
                ...
                */
                virtual void        ComputeDrawables(Vector2f &textSize, GL::DrawableArray &drawableArray, const Utils::Unicode::UTF32 &text);

                /** Destroy the font, to call at the end of the program to avoid memory leak. */
                static void         DestroyFonts();

            private:
                void    DrawVertices(Array<GL::DefaultVertexType::Textured2d> &vertices, unsigned int &noVertice, float X, float Y, float thickness, float italicCoeff, const Glyph *curGlyph, float factor);
                void    DrawUnderlines(Array<GL::DefaultVertexType::Colored2d> &underlines, unsigned int &noUnderline, float X, float Y, float thickness);

                void    TranslateCaraters(GL::DefaultVertexType::Textured2d *vertices, unsigned int noVertice, float offsetX, float offsetY);
                void    TranslateUnderlines(GL::DefaultVertexType::Colored2d *underlines, unsigned int noUnderline, float offsetX, float offsetY);
