...
void    PlainTextFormater::InitDrawables(GL::DrawableArray &drawableArray)
{
    // there is 2 drawables, one for the text and the other one for the underlined style.
    GL::GeometryBuffer<GL::DefaultVertexType::Textured2d,false> *geo1 = new GL::GeometryBuffer<GL::DefaultVertexType::Textured2d,false>(GL::Enum::Triangles);
    geo1->VBO().Init();// init the buffer to force it's creation and avoid it's done while we update the geometry

    GL::MaterialConfig *conf1 = new GL::MaterialConfig(GL::Blend::Alpha);
    conf1->Textures.InitSize(1);
    conf1->Textures[0] = _font->Bitmap();

    drawableArray.resize((_style.Enabled(Underlined)) ? 2 : 1);
    drawableArray[0] = new GL::Drawable(geo1, conf1);

    if (_style.Enabled(Underlined))
    {
        GL::GeometryBuffer<GL::DefaultVertexType::Colored2d,false> *geo2 = new GL::GeometryBuffer<GL::DefaultVertexType::Colored2d,false>(GL::Enum::Triangles);
        geo2->VBO().Init();// init the buffer to force it's creation and avoid it's done while we update the geometry
        drawableArray[1] = new GL::Drawable(geo2, new GL::MaterialConfig());
    }
    _needUpdate = true;
}

void    PlainTextFormater::ComputeDrawables(Vector2f &textSize, GL::DrawableArray &drawableArray, const Utils::Unicode::UTF32 &text)
{
    ...
    Array<GL::DefaultVertexType::Textured2d>      vertices(nbVertices);
    Array<GL::DefaultVertexType::Colored2d>       underlines;
    ...
    GL::GeometryBuffer<GL::DefaultVertexType::Textured2d,false> *geo = static_cast<GL::GeometryBuffer<GL::DefaultVertexType::Textured2d,false>*>(drawableArray[0]->Geometry);
    ...
    if (_style.Enabled(Underlined))
    {
        ...
        GL::GeometryBuffer<GL::DefaultVertexType::Colored2d,false> *geo = static_cast<GL::GeometryBuffer<GL::DefaultVertexType::Colored2d,false>*>(drawableArray[1]->Geometry);
...
void    PlainTextFormater::DrawUnderlines(Array<GL::DefaultVertexType::Colored2d> &underlines, unsigned int &noUnderline, float X, float Y, float thickness)
{
...
void    PlainTextFormater::DrawVertices(Array<GL::DefaultVertexType::Textured2d> &vertices, unsigned int &noVertice, float X, float Y, float thickness, float italicCoeff, const Glyph *curGlyph, float factor)
{
...
void    PlainTextFormater::TranslateCaraters(GL::DefaultVertexType::Textured2d *vertices, unsigned int noVertice, float offsetX, float offsetY)
{
...
void    PlainTextFormater::TranslateUnderlines(GL::DefaultVertexType::Colored2d *underlines, unsigned int noUnderline, float offsetX, float offsetY)
{
...
void    PlainTextFormater::ManageAlignment(bool &endWord, bool &endLine, float &X, float &Y, float thickness, float &curCharWidth, float &curWordWidth, float &sizeBetweenWords, float &lastSizeBetweenWords,
                                            ...
                                            Array<GL::DefaultVertexType::Textured2d> &vertices, unsigned int &noVertice,
                                            Array<GL::DefaultVertexType::Colored2d> &underlines, unsigned int &noUnderline)
{
